...
void main(){
    ...
    **/
    ...
    for (int i = -MIN_W/2 ; i <= MIN_W/2 ; i++)
    {
       for (int j = -MIN_W/2 ; j <= MIN_W/2 ; j++)
       {
            vec4 color  = texture2D(uNormalTexture,vec2(vCoordinate.x + float(i) * uOffsetX ,vCoordinate.y + float(j) * uOffsetY ));
            colors[index] = vec3(color.r,color.g,color.b);
            index ++ ;
       }
    }
    vec3 aaaaa = vec3(0.0);
    ...
    aaaaa /= float(index);
    vec4 nNormalColor = vec4(aaaaa,1.0);
